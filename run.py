...
# Code to check if the API is working
...
def calculate_surplus_data(sales_row):
    ...
    surplus_row = []
    for stock_num, sale_num in zip(stock_row, sales_row):
        surplus_num = int(stock_num) - sale_num
        surplus_row.append(surplus_num)
...
def get_stock_values(data):
    """
    Collects sandwich names from the stock worksheet,
    pair it with the relevant stock data and
    returns the data as a dictionary.
    """
    stock_values = {}
    headings = SHEET.worksheet("stock").row_values(1)
    for sandwich, stock in zip(headings, data):
        stock_value = {sandwich: stock}
        stock_values.update(stock_value)
    return stock_values
